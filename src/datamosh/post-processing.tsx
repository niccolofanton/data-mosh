...
/**
 * Deep link for the debug views: `?debug=motion`, `?debug=lost`, `?debug=frames`.
 *
 * Read once at module scope, which on a static export means "empty during
 * prerender, real in the browser". The value is only ever OR-ed into the
 * settings, never rendered, so there is nothing for hydration to mismatch on.
 *
 * The OR is what makes this work at all: the panel persists its state in
 * localStorage, so setting a schema default instead would be overwritten for
 * every returning visitor. The URL wins for the visit, the panel keeps its
 * memory, and the checkbox still turns the view back off.
 */
const DEBUG_PARAM =
...
export const PostProcessing = () => {
  ...
  const settings: DataMoshSettings = useMemo(
    () => ({
      ...
      debugMotion: pipeline.debugMotion || DEBUG_PARAM === "motion",
      debugLost: blocks.debugLost || DEBUG_PARAM === "lost",
      debugScale: pipeline.debugScale,
      debugFrames: pipeline.debugFrames || DEBUG_PARAM === "frames",
    }),
