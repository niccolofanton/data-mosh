import { lensRemapGlsl } from "@/found-footage/shaders/lens-remap";
 ...
 *             also what drives the image towards clipping (the "bloom")
 *
...
export const mainImageShader = /*glsl*/ `
    ...
    // The lens the picture was taken through, mirrored from the optics stage.
    // Both zero when the camera simulation is off, which makes every remap below
    // an exact identity.
    uniform float uBarrel;         // k1 of the radial distortion
    uniform float uSkew;           // rolling shutter shear

    ${lensRemapGlsl}

    /**
     * Inverse of the lens remap's Jacobian.
     *
     * Sampling the scene buffers through lensRemap puts the *lookup* in the
     * right place, but the vector that comes back is still expressed in the
     * undistorted frame: it says how far the content moved on the sensor, not
     * how far it moves across the picture. Where the lens compresses the image,
     * a given movement of the scene covers less of the screen, and the two
     * differ by exactly this factor.
     *
     * Taken by finite difference over one pixel rather than differentiated by
     * hand, so it stays right if the remap ever grows another term - and so
     * there is still only one place where the lens is defined.
     */
    mat2 lensJacobianInverse(vec2 uv) {
        vec2 texel = 1.0 / resolution;
        vec2 base = lensRemap(uv);

        // Columns: how the sampled position moves per unit of screen x and y.
        vec2 ddx = (lensRemap(uv + vec2(texel.x, 0.0)) - base) / texel.x;
        vec2 ddy = (lensRemap(uv + vec2(0.0, texel.y)) - base) / texel.y;

        float det = ddx.x * ddy.y - ddy.x * ddx.y;
        // A fold in the mapping would make this singular. Nothing the lens can
        // do produces one, but a zero determinant would blow the frame up.
        if (abs(det) < 1e-6) return mat2(1.0, 0.0, 0.0, 1.0);

        return mat2(ddy.y, -ddx.y, -ddy.x, ddx.x) / det;
    }
    ...
    uniform float uDebugLost;      // 1 = outline the lost-vector regions in white
    ...
    vec2 lostRegion(const in vec2 uv) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            // Distance to the nearest side of the rectangle, converted from
            // cell units to pixels, so the debug outline is one width whatever
            // the region's size.
            float edge = min((halfSize.x - d.x) / cells.x * resolution.x,
                             (halfSize.y - d.y) / cells.y * resolution.y);

            return vec2(1.0, step(edge, 1.5));
        }
     ...
     * summed again on every frame of the feedback chain, it is also the reason a
     * duplicated P-frame blooms towards white instead of merely deforming.
     *
    ...
    vec3 residualAt(const in vec2 uv, const in vec2 motion,
        ...
        vec2 p = uv - motion;
        vec3 lowPredicted = texture2D(pFrame, clamp(p + vec2(r.x, 0.0), 0.002, 0.998)).rgb;
        lowPredicted += texture2D(pFrame, clamp(p - vec2(r.x, 0.0), 0.002, 0.998)).rgb;
        lowPredicted += texture2D(pFrame, clamp(p + vec2(0.0, r.y), 0.002, 0.998)).rgb;
        lowPredicted += texture2D(pFrame, clamp(p - vec2(0.0, r.y), 0.002, 0.998)).rgb;
    ...
    void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
        ...
        //
        // Both of those buffers are rendered through the plain projection,
        // while this shader is working on a picture the lens has already bent.
        // The lookup therefore has to go through the same lens, and the vector
        // that comes back has to be brought into screen units - otherwise the
        // corners of a distorted frame are dragged by the motion of whatever
        // the scene had there before the glass moved it.
        vec2 sensorUV = lensRemap(mvUV);

        vec4 measured = texture2D(uVelocity, sensorUV);
        float useMeasured = uMotionSource * step(0.5, measured.a);

        vec2 motion = lensJacobianInverse(mvUV)
                    * mix(cameraMotionAt(sensorUV), measured.xy, useMeasured)
                    * uMotionGain * warp;
        ...
        float lost = lostRegion(blockUV).x * uBlockiness;
        ...
        // gesture drifts monotonically into saturation. That is the bloom.
