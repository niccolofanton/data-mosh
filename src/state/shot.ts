/**
 * Which of the three shots the edit is currently on.
 *
...
export type Shot = "room" | "dancer" | "cloth";

/** The rotation, in order. A cut advances by one and wraps back to the room. */
export const SHOTS: readonly Shot[] = ["room", "dancer", "cloth"];
...
> = {
  ...
  // Square on to the sheet and close enough that it fills the frame. The cloth
  // is the only thing in this shot and it is the only thing moving, so every
  // vector the effect measures is the wind.
  cloth: {
    position: [0, 0, 4.6],
    target: [0, 0, 0],
    fov: 45,
